...
#define _CHARS_(SELF) \
    ((SELF)->capacity > 0) ? (kk2_string_char_pt)((SELF)->chars) : (kk2_string_char_pt)&((SELF)->builtin)

#define _CHARS_EX_(SELF, OFFSET) \
    ((SELF)->capacity > 0) ? (kk2_string_char_pt)((SELF)->chars) + (OFFSET) : (kk2_string_char_pt)&((SELF)->builtin[(OFFSET)])

#define _MEMORY_(POINTER, COUNT) \
    (kk2_memory_t){ .pointer = (kk2_pointer_t)(POINTER), .count = (COUNT) }

#define _MEMORY_EX_(POINTER, OFFSET, COUNT) \
    _MEMORY_((kk2_string_char_pt)(POINTER) + (OFFSET), COUNT)

#define _CHARS_MEMORY_(SELF, COUNT) \
    _MEMORY_(_CHARS_(SELF), COUNT)

#define _CHARS_MEMORY_EX_(SELF, OFFSET, COUNT) \
    _MEMORY_(_CHARS_EX_(SELF, OFFSET), COUNT)
...
kk2_string_t kk2_string_with(
    ...
    if (view.count >= KK2_STRING_BUILTIN) {
        kk2_size_t capacity = self.behaviour.capacity(view.count);
        self.chars = self.alloc.new(capacity, sizeof(kk2_string_char_t));
        if (self.chars != NULL) {
            kk2_memory_copy(
                _MEMORY_(self.chars, capacity),
                _MEMORY_(view.chars, view.count),
                sizeof(kk2_string_char_t)
            );
        ...
        kk2_memory_copy(
            _MEMORY_(self.builtin, KK2_STRING_BUILTIN),
            _MEMORY_(view.chars, view.count),
            sizeof(kk2_string_char_t)
        );
        self.range.upper = view.count;
...
kk2_string_char_pt kk2_string_chars(
    kk2_string_cpt self
) {
    assert(self);
...
kk2_size_t kk2_string_length(
    ...
    assert(self);
    kk2_size_t result = 0;
    kk2_string_char_cpt chars = _CHARS_EX_(self, self->range.lower);
    while (*chars != '\0') {
        if ((*chars & 0xc0) != 0x80) {
            ++result;
        }
...
kk2_range_index_t kk2_string_range(
    kk2_string_cpt self
) {
    assert(self);
    return KK2_RANGE_INDEX(0, self->range.upper - self->range.lower);
}

kk2_index_t kk2_string_start_index(
    kk2_string_cpt self
) {
    assert(self);
...
kk2_index_t kk2_string_end_index(
    kk2_string_cpt self
) {
    assert(self);
...
kk2_string_view_t kk2_string_as_view(
    ...
    return (kk2_string_view_t){
        .chars = _CHARS_EX_(self, self->range.lower + range.lower),
        .count = count
    };
}
...
kk2_bool_t kk2_string_is_equal(
    ...
    return kk2_memory_is_equal(
        ...
        _MEMORY_(other.chars, other.count),
        sizeof(kk2_string_char_t)
    );
}
...
kk2_bool_t kk2_string_is_prefix(
    ...
    return kk2_memory_is_equal(
        ...
        _MEMORY_(find.chars, find.count),
        sizeof(kk2_string_char_t)
    );
}
...
kk2_bool_t kk2_string_insert(
    ...
    kk2_size_t old_capacity = (self->capacity > 0) ? self->capacity : KK2_STRING_BUILTIN;
    ...
    if ((index <= self->range.lower) && (self->range.lower >= insert.count)) {
        kk2_string_char_pt chars = _CHARS_(self);
        kk2_memory_copy(
            _MEMORY_EX_(chars, self->range.lower - insert.count, insert.count),
            _MEMORY_(insert.chars, insert.count),
            sizeof(kk2_string_char_t)
        );
        ...
        if ((new_count >= self->capacity) && (new_count >= KK2_STRING_BUILTIN)) {
            kk2_size_t new_capacity = self->behaviour.capacity(new_count) + 1;
            kk2_string_char_pt new_chars = self->alloc.new(new_capacity, sizeof(kk2_string_char_t));
            kk2_string_char_cpt old_chars = _CHARS_(self);
            if (new_chars != NULL) {
                kk2_size_t offset = 0;
                if (insert_index > 0) {
                    offset += kk2_memory_copy(
                        _MEMORY_(new_chars, insert_index),
                        _MEMORY_(old_chars, old_count),
                        sizeof(kk2_string_char_t)
                    );
                }
                offset += kk2_memory_copy(
                    _MEMORY_(new_chars, new_count),
                    _MEMORY_(insert.chars, insert.count),
                    sizeof(kk2_string_char_t)
                );
                kk2_memory_copy(
            ...
            kk2_string_char_cpt chars = _CHARS_(self);
...
kk2_bool_t kk2_string_fit(
    ...
    kk2_size_t count = self->range.upper - self->range.lower;
    if (count >= KK2_STRING_BUILTIN) {
        kk2_string_char_pt new_chars = self->alloc.new(count, sizeof(kk2_string_char_t));
        if (new_chars == NULL) {
            return false;
        }
        kk2_memory_copy(
            _MEMORY_(new_chars, count),
            _CHARS_MEMORY_EX_(self, self->range.lower, count),
            sizeof(kk2_string_char_t)
        );
        self->capacity = count;
